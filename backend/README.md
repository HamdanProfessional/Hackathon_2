...
1. **Authentication Tests** (`tests/api/test_auth.py`)
   ...
   - JWT token validation (expired, invalid signature, missing claims)
   - Input validation for email and password

2. **Task Management Tests** (`tests/api/test_tasks.py`)
   - CRUD operations for tasks
   - Ownership validation (users cannot access others' tasks)
   - Edge cases (special characters, max lengths, invalid data)
   - SQL injection protection

3. **Search and Filter Tests** (`tests/api/test_search_filter.py`)
...
Tests use SQLite in-memory database and automatically disable rate limiting:
...
# Run specific test
python -m pytest tests/api/test_auth.py::test_register_valid_user_succeeds -v
```
