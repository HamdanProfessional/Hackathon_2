...
1. **Authentication Components** (`components/auth/__tests__/`)
   - `LoginForm`: Validation, submission, error handling
   - `RegisterForm`: Form validation and user registration

2. **Task Components** (`components/task/__tests__/`)
   - `TaskCard`: Rendering, interaction, completion toggle
...
Tests use mocked API calls and Next.js router for isolation:
