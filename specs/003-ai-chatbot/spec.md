...
#### AI Integration and MCP Tools

- **FR-029**: System MUST implement MCP (Model Context Protocol) tools for task operations: add_task, list_tasks, update_task, delete_task, complete_task

- **FR-030**: System MUST expose chat endpoint at `/api/chat` with JWT authentication
...
- **FR-034**: System MUST validate user ownership before executing any task operation requested via chat

- **FR-035**: System MUST prevent prompt injection attacks where user tries to manipulate AI behavior to access other users' data

- **FR-036**: System MUST sanitize AI-generated content before displaying to prevent XSS or other injection attacks
